...
async def manage_trades_handler(
    ...
    async with uow:
        ...
        for forex_pair in forex_pairs:
            tasks.append(manage_trade_for_currency_pair(uow, indicator, forex_pair))

        await asyncio.gather(*tasks)


async def manage_trade_for_currency_pair(uow, indicator, forex_pair):
    ...
    trades: list[Trade] = await uow.trade_repository.get_open_trades_by_forex_pair(
    ...
    for trade in trades:
        ...
        half_spread_pips = (trade.half_spread_cost / gbp_per_pip) * pip_value * 1.25

        if trade.is_buy:
            adjusted_close_long = close
            new_stop = adjusted_close_long - multiplier * atr
            if new_stop > trade.stop and new_stop > (trade.close + half_spread_pips):
        ...
        if (trade.is_buy and trade.stop > (close + half_spread_pips)) or (
            not trade.is_buy and trade.stop < (close - half_spread_pips)
        ):
            logger.info(
