...
async def get_signal(refined_data: pd.DataFrame) -> pd.DataFrame:
    ...
    condition_close_above_MA = refined_data["close"] > refined_data["ShortTerm_MA"]
    ...
    rolling_close_below_MA_previous = condition_close_below_MA_previous.rolling(
        window_size
    ).sum()
    rolling_macd_below_signal_previous = condition_macd_below_signal_previous.rolling(
        window_size
    ).sum()
    ...
    rolling_close_above_MA_previous = condition_close_above_MA_previous.rolling(
        window_size
    ).sum()
    rolling_macd_above_signal_previous = condition_macd_above_signal_previous.rolling(
        window_size
    ).sum()
